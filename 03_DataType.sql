...
-- - 괄호안의 숫자가 자리수를 표시하며, 이는 바이트 수가 아닌 것을 꼭 기억하시기 바랍니다.

-- 2. 실수형 타입
-- - number(6, 2)는 소수점을 포함한 총 자리수가 6자리이고 소수점 둘째 자리까지 있는 실수형 값이 설정됩니다.
-- - 첫번째 인자값 6은 소수점 자리수를 포함한 총 자리수를 의미하고, 두번째 인자값 2는 소수점 자리 수를 의미합니다.
...
-- - 반대로 지정한 크기보다 적은 문자가 저장되려고 하면 실제 저장크기가 저장하려면 문자만큼 줄어들어서 저장됩니다.
-- - 기억장소가 절약되는 장점이 있습니다.
...
-- - 9. 지금 잘 사용되지 않는 자료형
-- - varchar : varchar2 와 같은 형식이지만 용량에서 varchar2가 업그레이드 되었으며, 이 때문에 잘 사용하지 않습니다.
